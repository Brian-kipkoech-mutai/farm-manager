...
const DashboardCn = ({ loadingParent, SetLoadingParent }) => {
  ...
  useEffect(() => {
    ...
    const fetchdata = async () => {
      ...
      const CleaneData = MappedData.map((data, i) => {
        ...
        const kilos = data
          .map((data) => data[day].kilo)
          .reduce((acc, kilo) => acc + Number(kilo), 0);
        const totalPrice = data
          .map((data) => data[day].price)
      ...
      const PriceWeek = CleaneData.reduce(
        (acc, { totalPrice }) => acc + Number(totalPrice),
        0
      );
      setTotalPriceWeek(PriceWeek);
      const totalKilosWeek = CleaneData.reduce(
        (acc, { kilos }) => acc + Number(kilos),
        0
      );
  ...
  return (
    <DashBoardPs
      {...{ finalData, totalPriceWeek, weekTotalKg, loadingParent }}
    />
  );
};
