...
const EntryDialoguePs = ({
  ...
  return (
    <Dialog
      ...
      open={isDialogueOpen}
      onOpenChange={setDialogueState}
    >
      <DialogTrigger onClick={() => setDialogueState(true)}>
        <div className="flex flex-col gap-2">
          <FontAwesomeIcon
            icon={faList}
            className="text-xl text-green-600 hover:text-green-500 active:text-green-900 hover:cursor-pointer"
          />
      ...
      <DialogContent>
        <DialogHeader className="flex  flex-col gap-2 ">
          ...
          <DialogDescription>
            {erroMsg ? (
              <span className="font-bold text-red-600 underline uppercase underline-offset-4 w-fit mx-auto block">
                {erroMsg}
              </span>
            ) : (
        ...
        <div className="w-full flex justify-between gap-2">
          <Button
            className="flex-1 bg-transparent hover:bg-green-100 text-slate-800  active:bg-green-800 active:text-white"
            onClick={handleCancel}
          >
            Cancel
          </Button>
          <Button
            className="flex-1 active:bg-green-800"
            onClick={() => {
              hadleSubmit();
            }}
          >
